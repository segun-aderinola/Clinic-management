...
if(isset($_POST['submit']))
  {
	 ...
	 if($sm_type == "syst" | $sm_gender == "syg" | $sm_workt == "sywt" )
	 {
		...
		if($row['smemail']==$sm_em){
           $emlduperr = '<br/ ><div style="font-size:12px" align="center" class="alert alert-danger">The Email address you entered is already been used!</div>';


		if($rowem['smtel']==$sm_cont){
           $conduperr = '<br/ ><div style="font-size:12px" align="center" class="alert alert-danger">The Contact Number you entered is already been used!</div>';

		   }
		}
        else{
		      $query = "INSERT INTO `staff`
		      (`staff_fname`, `staff_lname`, `smtype`, `smbd`, `smtel`, `smemail`, `smgender`, `smwoti`, `smaddr`,`insert_by`)
		       VALUES

...
{
	$_SESSION['noAccess'] = "<script> alert('Only Super Admin is given Access... Login as a Super admin Now!!'); </script>";
	header("location:patin.php");
}
?>
...
// removes backslashes
    ...
    $query = "SELECT * FROM `sup_admin` WHERE `sadusername`='$username'and `sadpassword`='".md5($password)."'";
        ...
        if($rows==1){
			...
			header("Location: patientmodify.php"); // Redirect user to patientmodify.php
            }else{

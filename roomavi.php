...
<div class="alert alert-info">By clicking Doctor Info can get information about Doctor, by clicking Patient Info can get information about admited patient, by clicking   Update Room Informtion can update room information</div>
  ...
  <?php
  $count=1;
  $sel_query="SELECT hospi_room.room_petID,hospi_room.eroom_drid,hospi_room.room_id,hospi_room.room_name,hospi_room.room_avilabl,hospi_room.room_drid,hospi_room.rc_time_mo,hospi_room.rc_time_ev,staff.staff_fname,staff.staff_lname FROM hospi_room LEFT JOIN staff ON staff.staffID=hospi_room.room_drid  ORDER BY `room_id` ASC";
  $result = mysqli_query($con,$sel_query);
  while($row = mysqli_fetch_assoc($result)) { ?>
  <tr valign="middle">

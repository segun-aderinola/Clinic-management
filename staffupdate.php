...
<h3 style="font-family:calibri;" class="text-center">Update Staff Information</h3></div>
</div>
</div>
</div>
...
<li style="font-family:calibri; font-size:16px;"><a href="staffmodify.php">Go Back!</a></ul>
</div>
</div>
</div>
<br />
...
`smwoti`='$wt'
WHERE `staffID` = '$id'";
$rqry = mysqli_query($con, $rquery) or die(mysqli_error($con));
// header("Location: staffmodify.php");
if($rqry)
{ ?>

  <script>
    alert('Data Update Successfull');
    window.location.href = 'staffmodify.php';
  </script>
...
<form class="" action="" method="post" id="upform">
  ...
  <dd style="font-size:12px;"><input pattern="[A-Za-z]{1,30}" title="Use Only Letters" name="pfname" id="patifn" style="margin:5px" class="form-control" type="text" value="<?php echo $row["staff_fname"]; ?>"></dd>

  <dt style="font-size:12px;"><strong>Staff Memeber Last Name : </strong> </dt>
  <dd style="font-size:12px;"><input pattern="[A-Za-z]{1,30}" title="Use Only Letters" name="psname" id="patsn"  style="margin:5px" class="form-control" type="text" value=" <?php echo $row["staff_lname"]; ?>"></dd>
  ...
  <input pattern="\d{11,11}"  maxlength="11" title="Invalid Mobile Number. Use only Letters and character count must be 11" required="required" name="ptel" id="tel"  type="text" class="form-control" value="<?php echo $row["smtel"]; ?>" placeholder="Username" aria-describedby="basic-addon1 ">
</dd>
...
<dl>
  ...
  <dd style="font-size:12px;">
  <select onchange="display()"  style="margin:5px; border-color:red;" id="fieldsac" required="required"  class="form-control" placeholder="Select your Staff Type">
